...
class Noh {
    public:
        ...
        inline Noh(): pos(), ant(), g(0.0), h(0.0), f(0.0) {}
        inline Noh(Coord Pos, Coord Ant, double G, double H): pos(Pos), ant(Ant), g(G), h(H), f(g + h) {}
        inline bool operator<(const Noh& outro) const {return f < outro.f;}
        inline bool operator==(const Coord& coord) const {return pos == coord;}
        inline bool operator!=(const Coord& coord) const {return !(pos == coord);}
    };
