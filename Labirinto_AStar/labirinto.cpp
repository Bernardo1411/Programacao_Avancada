...
double Labirinto::calculaCaminho(int& NC, int& NA, int& NF)
{
  ...
  Noh atual(
          ...
          Coord(),
          0.0,
          sqrt(pow(abs(orig.lin-dest.lin), 2) + pow(abs(orig.col-dest.col), 2))
      );
              ...
              for (dir.lin=-1; dir.lin<=1; dir.lin++) {
                  for (dir.col=-1; dir.col<=1; dir.col++) {
                      ...
                      if (dir != Coord(0,0)) {
                          ...
                          if (celulaLivre(suc.pos)) {

                              if (celulaLivre(Coord(atual.pos.lin + dir.lin, atual.pos.col)) &&
                                  celulaLivre(Coord(atual.pos.lin, atual.pos.col + dir.col))) {

                                  suc.ant = atual.pos;
                                  suc.g = atual.g + norm(dir);
                                  suc.h = sqrt(pow(abs(suc.pos.lin-dest.lin), 2) + pow(abs(suc.pos.col-dest.col), 2));
                                  suc.f = suc.g + suc.h;
                              }

                              else break;

                              for (lista=fechado.begin(); lista!=fechado.end(); ++lista) {
                                  if (suc.pos == lista->pos) {
                                      if (suc < *lista) {
                                          fechado.erase(lista);
                                          break;
                                      }
                              ...
                              for (lista=aberto.begin(); lista!=aberto.end(); ++lista) {
                                  if (suc.pos == lista->pos) {
                                      if (suc < *lista) {
                                          aberto.erase(lista);
                                          break;
      ...
      } while (!aberto.empty() && !solucao);

      if (!solucao) {
          cout << endl << "Nao existe caminho." << endl;
      }
      else {
          NC = 1;
          double distancia = atual.g;
          while (coordValida(atual.ant)) {
              for (lista=fechado.begin(); lista!=fechado.end(); ++lista) {
                  if (*lista == atual.ant) {
                      if (*lista != orig) Labirinto::set(lista->pos) = EstadoCel::CAMINHO;
                      atual = *lista;
                      NC++;
                      break;
                  }
